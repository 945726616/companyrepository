...
let dh_prime = '791658605174853458830696113306796803'
...
function expand (x, n) {
  let ans = int2bigInt(0, (x.length > n ? x.length : n) * bpe, 0);
  copy_(ans, x);
  return ans;
}
...
function mult (x, y) {
  let ans = expand(x, x.length + y.length);
  mult_(ans, y);
  return trim(ans, 1);
}
...
function sub (x, y) {
  let ans = expand(x, (x.length > y.length ? x.length + 1 : y.length + 1));
  sub_(ans, y);
  return trim(ans, 1);
}
...
function inverseMod_ (x, n) {
  let k = 1 + 2 * Math.max(x.length, n.length);
...
function eGCD_ (x, y, v, a, b) {
  let g = 0;
  let k = Math.max(x.length, y.length);
  if (eg_u.length != k) {
    eg_u = new Array(k);
    eg_A = new Array(k);
    eg_B = new Array(k);
