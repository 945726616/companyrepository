...
var fdSliderController = (function() {
	...
	var parseCallbacks = function(cbs) {
		...
		for(var i = 0, fn; fn = cbs[i]; i++) {
			type = fn.match(/(fd_slider_cb_(update|create|destroy|redraw|move|focus|blur|enable|disable|handle)_)([^\s|$]+)/i)[1];
	...
	var init = function( elem ) { // 初始化滑块
		var ranges     = /fd_range_([-]{0,1}[0-9]+(d[0-9]+){0,1}){1}_([-]{0,1}[0-9]+(d[0-9]+){0,1}){1}/i,
			...
			callbacks  = /((fd_slider_cb_(update|create|destroy|redraw|move|focus|blur|enable|disable|handle)_)([^\s|$]+))/ig,
	...
	function fdSlider(options) {
      ...
      from_user = true;
		...
		function destroySlider() {
			...
			wrapper = bar = cover = handle = outerWrapper = timer = null;
			callback("destroy");
			callbacks = null;
		};
		...
		function setCoverColor(){
			...
			bW = outerWrapper.offsetWidth - 20;         //for IE6 原始bW = bar.offsetWidth
			bH = bar.offsetHeight;

			if(vertical){
				bar.style.height = bH * val / range + "px";
				cover.style.height = bH * (range - val) / range + "px";
				cover.style.top = parseInt(bar.offsetLeft) + bH * val / range + "px";
			}else{
				bar.style.width = (bW * parseInt(val) / range + 3<0) ? 0 : bW * parseInt(val) / range + 3 + "px";  
				//3 to overlap
				cover.style.width = ((bW * (range - parseInt(val)) / range <0) ? 0 : bW * (range - parseInt(val)) / range)+ "px";
				cover.style.left = parseInt(bar.offsetLeft) + bW * val / range + "px";
			}
		};
		function callback(type) {
			var type;
			var cbObj = {"elem":inp, "value":tagName == "select" ? inp.options[inp.selectedIndex].value : inp.value, "dest":Math.floor(targetPos)};
			if(from_user == true && type in callbacks) {
		...
		function trackMouseWheel(e) {
			...
			if(delta) {
				...
				targetPos = min + (Math.round(Math.min(Math.max(xtmp, 0), maxPx) / stepPx) * inc);
				callback("handle");
		...
		function trackMouse( e ) {
			e = e || window.event;
			targetPos = min + (Math.round(snapToNearestValue(handlePos + (vertical ? e.clientY - mousePos : e.clientX - mousePos)) / stepPx) * inc);
			callback("handle");
		...
		function onMouseDown( e ) {
			...
			targetPos =  min + (Math.round(posx / stepPx) * inc);
			if(useTween) {
				tweenTo(posx);
			} else if(clickJump) {
				pixelsToValue(posx);
		...
		function incrementHandle(numOfSteps,from) {
			var value = tagName == "input" ? parseFloat(inp.value) : inp.selectedIndex;
			if(isNaN(value) || value < Math.min(min,max)) value = Math.min(min,max);
		...
		(function() {
			...
			cover.className           = "fd-slider-cover";
			...
			outerWrapper.appendChild(cover);
		...
		return {
			...
			increment:      function(n,f) { incrementHandle(n,f); },
	...
	return {
		...
		increment:              function(id, numSteps,from) { if(!(id in sliders)) { return false; }; sliders[id].increment(numSteps,from); },
		addEvent:               addEvent,
