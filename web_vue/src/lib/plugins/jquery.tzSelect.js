(function($){
	...
	$.fn.tzSelect = function(options,isEdit){
        var me = this;
            if(hack_tzSelectCss)
            {/* hack by kugle for < IOS under 6.0 */
		...
		this.each(function(){
			...
			dropDown.bind('show',function(){
                ...
                {
                    $("#manager_page").mCustomScrollbar("update");
                    mx(".dropDown", me[0].parentNode)[0].style.height = 320 + "px";
                    if(!isScrollarCreate){
                        $(mx(".dropDown", me[0].parentNode)[0]).mCustomScrollbar();
                        isScrollarCreate=1;
                    }
                }
				...
				dropDown.slideDown("fast", function(){
				    if(me.find("option").length > 12&& isEdit!=1)
                    {
                        $(mx(".dropDown", select[0].parentNode)[0]).mCustomScrollbar("update");
                    }
                    var wifiConnection;//wifi连接状态
                    if(mx("#ssid_status")){
                        for(var i=0;i<mcs_connect_fail.length;i++){
                        if((mx("#ssid_status").title ==mcs_connect_fail[i])){wifiConnection=15;break;}else{wifiConnection=20;}}}
		       ...
		       // return false;
                ...
                {
                    $("#manager_page").mCustomScrollbar("update");
                    //$(mx(".dropDown", me[0].parentNode)[0]).mCustomScrollbar("destroy");
                }
			...
			function getOptionText(text){
			 ...
			 $(mx(".dropDown", me[0].parentNode)[0]).click(function(e){});

		});
        /*if(this.find("option").length > 8)
        {
            mx(".dropDown", this[0].parentNode)[0].style.height = 200 + "px";
            $(mx(".dropDown", this[0].parentNode)[0]).mCustomScrollbar();
        }*/
	}
})(jQuery);
