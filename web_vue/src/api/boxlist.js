  ...
  boxlist_img_get (params) {
    ...
    for (let i = 0; i < data_ipc_length; i++) {
      ...
      if (store.state.jumpPageData.localFlag) {
        ...
        let url = window.location.protocol + "//" + window.location.host + "/api/ccm/ccm_pic_get.js?dsess=1&dsess_nid=" + login.create_nid() + "&dsess_sn=" + params.sn + "&dtoken=" + token + "&dflag=2";
