...
export default {
  methods: {
    ...
    create_set_page (obj) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          for (let j = 0; j < content.length; j++) {
            menu_content +=
              ...
              + "<div class='new_menu_list_img' style='background:url(./imgs/device/set_" + content[j].type + ".png) no-repeat;width:34px;height:34px;background-size:100% 100%'></div>"
      ...
      // ********************* //
      // 创建设置右栏内容
      function create_right_page (info) {
        // 判断是否含有联动标识
        if (new_ealf && info.type === "record") {
          info.type = "record_new"
        }
        if (new_ealf && info.type === "alarm_device_tips") {
          info.type = "alarm_new"
        }
        if (new_ealf && info.type === "alarm") {
          info.type = "alarm_new"
        }
        switch (info.type) {
          case "about": {
          ...
          case "network": {
            ...
            function ccm_set_network_info_request (obj) {
              ...
              node = msdk_agent.devs.get(node_sn);
              if (!node) return;
              ...
              if (net_info) {
                ...
                l_dom_select_nic.change_value = function (obj) {
                  ...
                  $(l_dom_button_setup).bind("click", function () {
                    ...
                    if (_this.$store.state.jumpPageData.selectDeviceIpc && g_network_environ == "private")          //Direct Connect
                    {
                    ...
                    if (ip_refresh) {
                      let type = msdk_agent.devs.get(_this.$store.state.jumpPageData.selectDeviceIpc).type;
            ...
            function get_network_ack (msg, ref) {
              ...
              deviceType = msdk_agent.devs.get(_this.$store.state.jumpPageData.selectDeviceIpc).type
              if (net_info) {
                let i, length = net_info.length, inner_html = "";
                for (i = 0; i < length; ++i) {
                  if (net_info[i].token == "eth0") {
          ...
          case "sdcord": {
            info.dom.innerHTML =
              ...
              + "<div id='sd_export_link_content'>" + mrs_login_please + "www." + g_oems + ".com," + mrs_sd_export_tips + "</div>"
          ...
          case "accessory": {
            ...
            function accessory_create_move_page (obj, index) {
              ...
              let l_dom_attachmen_box_close,
                l_dom_attachmen_event_ico;
              let data = obj[1].dev[index];
              let l_obj = obj;
              let l_attachmen_id = data.id;
              let dev_type = data.type, scene_class, dev_type_name, dev_video_set_page;
              let in_ico = "", out_ico = "";
              let dev_f;
              let in_num, out_num;
              let motion_track = g_motion_track;
            ...
            function create_scene_result_page (obj) {
              ...
              if (obj.result) {
                content_img = "background:url(imgs/device/success.png) no-repeat;";
                content_text = mcs_add_successfully;
                content_btn = mcs_ok;
              } else {
                content_img = "background:url(imgs/device/fail.png) no-repeat;"
            ...
            function create_scene_search_page (obj) {
              ...
              let time_num = 0;
              function search_peripheral (msg, ref) {
                let list_img = "";
                if (type == 5) {
                  list_img = "background:url(imgs/device/add_sos.png) no-repeat;";
                } else if (type == 6) {
            ...
            function scene_get_ack (msg, ref) {
              ...
              for (let i = 0; i < l_scene_devs.length; i++) {
                ...
                let dev_type = l_scene_devs[i].type, scene_class, dev_type_name, dev_f;
                let in_ico = "", out_ico = "";
                if (dev_type == 1) {
            ...
            function create_set_record_page (repeat_page) { //允许录像设置开关时间页面
              ...
              function schedule_get_ack (msg) {
                ...
                if (msg && msg.result == "") {
                  ...
                  let l_data_2 = mcodec.b64_2_binary(msg.data.sch.schedule, 1), str = "", arr = [], arr2 = [], arr_tmp = [];
                  if (!l_data_2) return;
          ...
          case "system": { // 系统
            ...
            l_inner_html = // 为mipc添加system_class判断样式
              ...
              + "<div class='options_float_left' id='system_upgrade_left' style='" + ((g_now_lang == 'ja' || g_now_lang == 'ru') ? "width:132px" : "") + "'></div>" //在线升级
          ...
          case "others": { // 其他
            ...
            function others_cam_get_ack (msg) {
              ...
              let param = Array();
              param = msdk_agent.devs.get(_this.$store.state.jumpPageData.selectDeviceIpc).p;
                  ...
                  _this.publicFunc.mx("#set_time_confirm_btn").onclick = function () {
                    ...
                    if (start_time >= end_time) {
                      ...
                      if (index == "add") {
                        ...
                        g_total_data[index].day = g_select_week
            ...
            function plan_record_get_ack (msg) {
              ...
              if (msg.result == "") {
                ...
                if (msg.data.sche.plan) {
                  ...
                  record_plan_continue.sche_form = sche_format(msg.data.sche.plan)
                  if (record_select.length > 0) {
                    record_plan_continue.alarm_status = "on"
                    record_select = time_deal(record_select)
                    record_select.forEach(function (item, index) {
                      let day_arr = item.day.split("、")
                      item.day = change_day_to_arr(day_arr)
                    })
                    record_plan_continue.plan = record_select
                  } else {
                    record_plan_continue.alarm_status = "off"
                    record_plan_continue.plan = []
                    record_plan_continue.sche_form = sche_format(record_plan_continue.plan)
                  }
                } else {
                  record_plan_continue.alarm_status = "off"
                  record_plan_continue.plan = []
            ...
            function new_set_record_alarm () {
                  ...
                  if ($("#at_home_btn").attr("type") == "false") {
                    if (dev_name == 'c_record') {    //持续录像设置录像开关和 计划表
                      ...
                      sche_form.forEach(function (item, index) {
                        ...
                        for (let i = 0; i < temp.length; i++) {
                          (function (i) {
                            if (change_string_to_four_bit_arr(temp[i])[arr_flag_index] == 1) {
                              temp[i] -= cut_flag
                            }
                          })(i)
                        }
                      ...
                      let plan_sel = sche_trans_to_second_format(sche_form)
                    ...
                    for (let i = 0; i < sche_form.length; i++) {
                      ...
                      for (let j = 0; j < h_arr_temp.length; j++) {
                        (function (j) {
                          if (change_string_to_four_bit_arr(h_arr_temp[j])[arr_flag_index] == 1) {
                      ...
                      for (let j = 0; j < day_arr.length; j++) {
                        (function (j) {
                          if (j >= temp.start && j < temp.end) {
                            if (change_string_to_four_bit_arr(day_arr[j])[arr_flag_index] == 0) {
                              day_arr[j] += add_flag
                            }
                            else {
                              day_arr[j] += 0
              ...
              function record_new_plan_log () {
                ...
                for (let i = 0; i < g_total_data.length; i++) {
                  ...
                  let alarm_day_time = alarm_timeArr_to_word(g_total_data[i].day)
                  l_dom_set_out_time_box.innerHTML +=
                ...
                $(".selsect_set_time_btn").each(function (index) {
                  $(this).click(function () {
                    g_select_week = g_total_data[index].day
      ...
      // ************************************* 联动框架公共函数 ************************************* //
      function sche_format (sche) { // 生成7*24小时计划表
        // // console.log(sche)
        let start_h = ''
        let end_h = ''
        let start_day = ''
      ...
      function change_string_to_four_bit_arr (str) { //将字符串转换成四位二进制字符串
      ...
      function sche_trans_to_second_format (sche) { //将7*24小时计划表转换成秒的形式
        let plan_final = []
        let plan_info = []
        let start_index = 0
        sche.forEach(function (item, index) {
      ...
      function sche_add_action_name (sche, type) { // 计划表附带上动作
        let control_time = 0
        ...
        sche.forEach(function (item, index) {
          if (item.flag != 0 && item.index == 0) {
            let flag_arr = change_string_to_four_bit_arr(item.flag)
            let action_name = []
      ...
      function change_day_to_arr (arr) {
        let week_standard = [
          mcs_Sunday_and,
          mcs_Monday_and,
          mcs_Tuesday_and,
      ...
      function alarm_timeArr_to_word (arr) {
        let week_standard = [mcs_Sunday_and, mcs_Monday_and, mcs_Tuesday_and, mcs_Wednesday_and, mcs_Thursday_and, mcs_Friday_and, mcs_Saturday_and]
        let day_str_temp = []
        let day_str = ""
        arr.forEach(function (item, index) {
      ...
      function record_flag (data1, data2) {
        let a = 0, b = 0, c = 0, d = 0;
        let f = [];
        let g = [];
        let week_flag = 'true';
  ...
  async mounted () {
    ...
    await this.create_set_page({ parent: $('#set') }) // 进入页面后加载
