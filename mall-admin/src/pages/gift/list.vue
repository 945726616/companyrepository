<template>
  <div class="product">
    ...
    <div class="center-options">
      ...
      <el-button type="primary" @click="toAddOrUpdate">创建礼包</el-button>
    </div>
    <el-table
      :data="tableData"
      ref="multipleTable"
...
export default {
  ...
  methods: {
    ...
    ...mapActions(['setEditVipProd']),
    // 编辑
    edit (row) {
      this.setEditVipProd(row)
      this.$router.push('/gift/addOrUpdate?type=edit')
    },
    // 查询分类
    async getCategory () {
      let data = {
    ...
    toAddOrUpdate () {
      this.$router.push('/gift/addOrUpdate?type=add')
    },
    search () {
      this.total = 0
      this.page = 1
