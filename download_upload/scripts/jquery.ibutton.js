...
;(function($){
    ...
    $input
    	...
    	.after(
      	...
      	+ '<div class="' + options.classLabelOff + '"><span><label title=' + options.labelOff + '>'+ options.labelOff + '</label></span></div>'
      	+ '<div class="' + options.classLabelOn + '"><span><label title=' + options.labelOn + '>' + options.labelOn   + '</label></span></div>'
			);
    ...
    // automatically resize the control
		...
		var positionHandle = function (animate){
			...
			if( animate && options.enableFx ){
				...
				$handle.css("left", x);
				$onlabel.css("width", $onlabel[0].clientWidth ? $onlabel[0].clientWidth : (x + 4));
