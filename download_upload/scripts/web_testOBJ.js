(function(window) {
    ...
    var g_auto_refresh_dev,g_select_dev,g_select_default_or_input_pwd = false,g_refresh_device_button,g_input_pwd,g_all_data = [],g_play_cell = [],g_definition;
                ...
                function get_local_dev_req(num){
                    ...
                    if(data.status==0){
                        function mme_send(counts){
                            ...
                            var localVal=g_mlocal_storage.get("ip_range");
                            var b;
                            if(localVal && eval('(' + localVal + ')').checked)
                            {
    ...
    function layoutCell() {
        ...
        var paddingLeft = (getDocW - 500 * 3) / 3;
    ...
    function create_plug_search_dev_list(data){
        ...
        function cellClick(cellMsg){
            ...
            refresh_dev_frame.contentWindow.message=function(msg){
                ...
                if(!dev_get(msg.data.SerialNumber)) {
                    ...
                    connetIPC(num,cellMsg);
                    g_devsNum++;
                }
            };
        }

        function connetIPC(num,msg) {
            console.log("connet_ipc",num,msg);
            ...
            localStorage.setItem("dev_pass",g_default_pass_input.value);
        ...
        jQuery(document).keydown(function (event) {
            ...
            switch (event.keyCode) {
                case 13:
                    cellClick(g_play_cell[currentIndex]);
                    if (jQuery("#popView").css("display") === "block"){
                        jQuery("#surePop").click();
                    }
                    break;
                case 27:
                    if (jQuery("#page_top_two").css("display") === "block"){
                        click_back();
                    }
                    if (jQuery("#popView").css("display") === "block"){
                        if (jQuery("#popView")){
        ...
        function create_dom_device_li(){
            for (var num = 0;num < data.length;num++){
                g_device_list_ul.innerHTML +=
                    "<li id='"+ "device_list_li" + num + "' class='device_list_li' index='"+num+"' ip='"+data[num].ProbeMatch[0].XAddrs
                    +"' title='IP:"+data[num].ProbeMatch[0].XAddrs+"' style='cursor:pointer;"+(data[num].sn==g_select_dev?"color:#A8FF24;":"color:#fff;")
                ...
                new PlayCell({
                    device : {
                        ip : data[num].ProbeMatch[0].XAddrs,
                        name : data[num].sn,
                    ...
                    allData : function (msg) {
                        ...
                        // get_wifi_info(msg.id,msg.index);
                    },
                    oncall : function (msg) {
                        cellClick(msg);
                    },
    ...
    function create_search_dev_frame(requestIP,index) {
        ...
        refresh_dev_frame.contentWindow.message=function(msg){
            if(!dev_get(msg.data.SerialNumber)) {
                ...
                jQuery(".dev_os_version").eq(index).html(msg.data.Version?msg.data.Version:"v1");
            }
        };
    }
    ...
    function get_peripheral(){
        ...
        function exdev_discover_ack(msg,ref){
            // console.log(msg,"exdev_discover_ack");
            if(msg&&msg.result==""){
                loop_test_exdev(0)
            }else{
                add_test_li("peripheral",2);
    ...
    // $$("click_back").onclick = function(){test('click_back')};
})(window); 