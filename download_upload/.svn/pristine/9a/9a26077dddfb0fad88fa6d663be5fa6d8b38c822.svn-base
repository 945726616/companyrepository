(function(window, document){
    login_data={};
    to_list=['cdms','cpis','cbms','ctck','0x50000000','cmall','test'];
    ...
    'cmall_review_comment_req','cmall_get_comment_req'],
                    test:['test_req']};
   /* all_value_list={cmall:{cmall_set_addr_req:['flag','addres','id','name','country','province','address','postcode','phone','landline'], cmall_get_addr_req:['flag'],
    ...
    all_value_list={cmall:{cmall_set_addr_req:{flag:"int32",addres:{id:"int32",name:"lenstr",country:"lenstr",province:"lenstr",address:"lenstr",postcode:"lenstr",
        phone:"lenstr",landline:"lenstr"}}, cmall_get_addr_req:{flag:"lenstr"},
        cmall_get_cart_req:{start:"int32",counts:"int32",language:"lenstr"},cmall_set_cart_req:{flag:"int32",id:"int32",serial_id:"lenstr",modle_id:"lenstr",
        ...
        {flag:"uint32",month:"uint32",start:"uint32",counts:"uint32"},cmall_create_order_req:{},cmall_pay_req:{order_id:"uint32",pay_type:"uint32"},cmall_update_pay_req:{
            debug_pass:"lenstr",pay_id:"lenstr",order_id:"uint32"},cmall_check_order_req:{start:"uint32",counts:"uint32",month:"uint32",user:"lenstr",status:"lenstr"},
        ...
        cmall_get_comment_req:{start:"uint32",counts:"uint32",flag:"uint32",serial_id:"lenstr"}},
        test:{test_req:{test_1:[{test_1_1:"lenstr"}]}}
    };
    all_value_list.cmall.cmall_create_order_req.product=[all_value_list.cmall.cmall_set_stock_req.stock_total];
    ...
    function fork() {
        g_page.innerHTML='<div id="login_page">'+
            '<h1>test page</h1>' +
            '<textarea name="textfield3" cols="30" rows="3" id="data_text"></textarea>'+
            '<div id="Parameters">' +
            '<h2>Parameters</h2>' +
            '<button id="add_Parameters">添加参数</button><br>'+
            '<button id="del_Parameters">删除参数</button><br>'+
            '</div>' +
            '<div id="address">' +
            '<h2>address</h2>' +
            '</div>' +
            '<button id="ture_Parameters">锁定参数</button>'+
            '<button id="ture_req">锁定请求</button>'+
            '<button id="nid_e">有nid</button><br>'+
            '<button id="send_msg">发送</button><br>'+
        ...
        document.getElementById("add_Parameters").onclick=function(){
           // add_parameters(all_data);
            build_parameters(all_data,value_list,"","",function(d) {
                all_data = d;
                var dddd=JSON.stringify(all_data);
        ...
        document.getElementById("ture_Parameters").onclick=function() {
        ...
        document.getElementById("ture_req").onclick=function() {
        ...
        function build_parameters(data,parameters,len,kkk,callback) {
         ...
         //   console.log(dd);
            if (isArrayFn(data) == true) {
                var arr={}
                tmp.innerHTML += len + '<button id="save_arr_'+kkk+'">存入</button><button id="clear_arr_'+kkk+'">清空</button><br>';
